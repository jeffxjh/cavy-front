...
export default {
  ...
  methods: {
    ...
    beforeUploadVideo (file) {
      const fileType = file.name.endsWith('.flv') || file.name.endsWith('.avi')
      || file.name.endsWith('.mp4')|| file.name.endsWith('.ogg')|| file.name.endsWith('.wmv')|| file.name.endsWith('.rmvb')

      console.info(file,fileType,"file")
      const isLt300M = file.size / 1024 / 1024 < 300
      if (['video/mp4', 'video/ogg', 'video/flv', 'video/avi', 'video/wmv', 'video/rmvb'].indexOf(file.type) === -1&&!fileType) {
